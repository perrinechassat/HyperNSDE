...
# 		if use_delta_t:
# 			self.input_dim += 1
# 		self.gru_rnn = nn.modules.rnn.GRU(self.input_dim, self.gru_rnn_output_size).to(device)
...
# 		prev_y = torch.zeros((1, n_traj, self.latent_dim)).to(data.device)
# 		prev_std = torch.zeros((1, n_traj, self.latent_dim)).to(data.device)
...
# 				ode_sol = prev_y + inc
# 				ode_sol = torch.stack((prev_y, ode_sol), 2).to(data.device)
