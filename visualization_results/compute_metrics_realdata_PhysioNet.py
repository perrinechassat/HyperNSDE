...
def clean_outliers(syn, threshold=1e5, verbose=True):
    """
    Remove samples where any value in X or W exceeds threshold in absolute value.
    Filters along the patient (N) dimension.
    """
    X = syn["X"]  # (N, T, V)
    W = syn["W"]  # (N, d_W) or None

    # Bad mask: any time point or variable exceeds threshold
    bad_X = (X.abs() > threshold).any(dim=-1).any(dim=-1)  # (N,)
    bad = bad_X

    if W is not None:
        bad_W = (W.abs() > threshold).any(dim=-1)           # (N,)
        bad = bad | bad_W

    good = ~bad
    n_removed = bad.sum().item()

    if verbose and n_removed > 0:
        print(f"    Removed {n_removed}/{X.shape[0]} samples with values > {threshold:.0e}")

    cleaned = {
        "X": X[good],
        "M": syn["M"][good] if syn.get("M") is not None else None,
...
def load_syn_one_gen(path, gen_idx, n_per_gen, scales, method_scaling="standard"):
    ...
    # out = clean_outliers(out, threshold=1000)
    if out["T"].ndim > 1:
        out["T"] = out["T"][0]
...
if __name__ == "__main__":
    ...
    # 2. Configuration
    metric_config = {
        # "longi.MMD": {"kernel": "sig rbf", "sigmas": [0.5, 1, 10, 20, 60, 100, 200, 400, 800], "dyadic_order":1, "lead_lag":False, "subtrack_init_point":False, "scale_time":True},
        "longi.MMD": {"kernel": "sig rbf", "sigmas": [0.5, 10, 100, 400], "dyadic_order":1, "lead_lag":False, "subtrack_init_point":False, "scale_time":True},
        ...
        "prediction_score": {"predictor_type": "S4", "filling_type": 'last', "verbose": False, "input_len": 1000}, # "future_steps": 5,
    ...
    # for i in range(gen_idx_block*10, (gen_idx_block+1)*10):
    gen_idx = gen_idx_block
    # n_gen_syn = 50
    rows = []
    ...
    for syn_file in syn_files:
        ...
        df = eval_engine.evaluate_several_synthetic_sets(
            ...
            metrics_adapt_multinodes=["longi.MMD", "longi.pairwise_corr", "global_pairwise_corr", "prediction_score", "NNAA", "MIR"]
        )
        df["model"] = model_name
        df["gen_idx"] = gen_idx
        rows.append(df)  
