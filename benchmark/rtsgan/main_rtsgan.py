...
parser.add_argument("--ae-batch-size", default=128, dest="ae_batch_size", type=int, help="Minibatch size for autoencoder")
parser.add_argument("--gan-batch-size", default=512, dest="gan_batch_size", type=int, help="Minibatch size for WGAN")
...
parser.add_argument("--N-generated-datasets", default=50, dest="N_generated_datasets", type=int,
...
N = options.N_generated_datasets
...
for i in range(N):
    logger.info(f"Generating dataset {i+1}/{N}...")
    result = syn.synthesize(n_test)
    dataset["synthetic"] = result
    out_path = f"{root_dir}/synth_{i+1}.pkl"
    with open(out_path, "wb") as f:
        pickle.dump(dataset, f)
    logger.info(f"Saved {out_path}")
    del result
    dataset.pop("synthetic")  # don't let it accumulate in dataset dict
